...
/**
 * Demo13 - 延迟订单消费者（方案一：插件）
 *
 * 职责：从延迟队列消费 = 订单已到超时时间 → 查订单状态 → 未支付就关单
 *
 * ============================================================
 *  消息流向（站在 Consumer 视角）：
 *  ============================================================
 *  Producer → 延迟交换机(插件按 x-delay 延迟) → 延迟队列 → 【本 Consumer】
 *  收到消息即代表「距下单已过指定延迟时长」，该检查订单状态了
 *
 * ============================================================
 *  业务逻辑（延迟关单核心，生产落地三件套）：
 *  ============================================================
 *  1. 查订单状态：从延迟队列收到消息 = 订单已超时
 ...
 *  3. MQ 不可靠兜底：插件方案依赖 Broker，宕机 / 消息丢失会漏关
 ...
 *  2. 幂等：死信/延迟消息可能重复投递，关单动作必须幂等（条件更新天然幂等）
 *  3. 并发：支付回调与关单任务并发，用状态机条件更新保证不会「已支付被关单」
 *  4. 兜底：MQ 不可靠时定时任务扫表兜底，防止漏关
 *
 * @author wang
 * @date 2026-08-18
 */
public class Consumer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (Connection conn = ConnectionUtil.createConnection();
            ...
            // 4. 注册消费者到延迟队列（手动 ACK）
            System.out.println(" [*] 等待消费延迟队列 " + DelayPluginTopology.DELAY_QUEUE + "（按 Ctrl+C 退出）");
            ch.basicConsume(DelayPluginTopology.DELAY_QUEUE, false, new DefaultConsumer(ch) {
                ...
                public void handleDelivery(String consumerTag,
                    ...
                    try {
                        System.out.println(" [" + LocalTime.now() + "] [x] 收到延迟消息（订单已超时）：" + orderId);
