...
/**
 * 消费端：负责「消息不丢」里消费者这一半 —— 手动 ACK + 幂等 + 失败进死信。【标准实现】
 *
 * 两个结构上的选择：
 *   1. @RabbitListener 标在方法上而不是类上。
 *      @RabbitHandler 是为"一个类里按 payload 类型分派多个方法"设计的，
 *      这里只有一个方法，留着它只会让 Message/Channel 参数解析多绕一层。
 *   2. 参数是 (Message, Channel) 而不是 String。
...
public class ReliabilityConsumer {
     ...
     * 这里用 newKeySet().add() 的返回值，同样是一步原子。
     */
    ...
    public void receive(Message message, Channel channel) throws IOException {
        ...
        try {
            // ---- 1. 幂等判断：重复消息直接丢弃并 ack ----
            // 为什么必须幂等：broker 只保证"至少一次"，网络抖动 / 消费者 ack 前宕机
            // 都会导致同一条消息被重投，业务必须能扛住重复。
            // add() 本身就是"不存在则插入并返回 true"，一步原子，别拆成 contains + add。
            ...
            // ---- 2. 业务处理 ----
            // 约定：body 含 "FAIL" 就抛异常，模拟业务失败（对应 Producer 的 sendFail）
            ...
            channel.basicAck(tag, false);
            log.info("[Consumer] basicAck tag={}", tag);
            ...
            //
            // ⚠️ 坑 1：requeue=true 是经典事故 —— 业务永远失败 → 无限回队 → 死循环打满 CPU。
            //          正确做法：requeue=false + 死信队列兜底 + 人工/定时补偿。
            // ⚠️ 坑 2：失败必须把 msgId 从幂等表里**移除**（release）。
            //          否则死信补偿重投时会被当成重复消息直接吞掉，消息等于永久丢弃。
            try {
                if (msgId != null) {
                    processedIds.remove(msgId);
                }
                channel.basicNack(tag, false, false);
                log.warn("[Consumer] basicNack(requeue=false) tag={} -> 进死信队列", tag);
            } catch (IOException io) {
                log.error("[Consumer] nack 失败 tag={} err={}", tag, io.getMessage());
            }
        }
    }
}
