...
/**
 * Demo04 - 路由（Direct）消费者
 * 职责：创建临时队列绑定到 direct exchange，只接收指定 routing key 的消息
 * 特点：通过 queueBind 指定 binding key，只接收匹配的消息
 */
public class ReceiveLogsDirect {
    ...
    static void main() throws IOException, TimeoutException {
        ...
        // 5. 把临时队列绑定到 exchange，指定 binding key 为 "error"
        //    含义：我只关心 routing key="error" 的消息
        //    如果 EmitLogDirect 发送 routing key="info"，这个消费者收不到
        channel.queueBind(queue, EmitLogDirect.EXCHANGE_NAME, "error");
